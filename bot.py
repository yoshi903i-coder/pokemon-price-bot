...
HIGH_RARITY = [
    "RR","RRR","SR","SSR","SAR","SA",
    "HR","UR","MUR","AR","CHR","CSR",
    "MA","BWR","TR","S","P"
]
...
try:
    ...
    while True:
        ...
        for item in items:
            ...
            rarity_match = re.search(r'【([^】]+)】', full_name)
            rarity = rarity_match.group(1) if rarity_match else ""
            if "状態" in rarity:
                rarity_match2 = re.search(r'【([^】]+)】(?!.*【状態)', full_name)
                rarity = rarity_match2.group(1) if rarity_match2 else ""
